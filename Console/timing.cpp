...
std::ostream& operator<<(std::ostream& f, const Periode& p)
{
    p.afficher(f);
    return f;
}
...
void Date::setDate(unsigned short int j, unsigned short int m, unsigned int a)
{
    ...
    if (a <= 3000)
        ...
        throw TimeException("erreur: annee invalide");
    if (m >= 1 && m <= 12)
        mois = m;
    else
        throw TimeException("erreur: mois invalide");
    switch (m)
    {
        case 1:
        case 3:
        ...
        case 12:
            if (j >= 1 && j <= 31)
                ...
                throw TimeException("erreur: jour invalide");
            break;
        case 4:
        case 6:
        case 9:
...
void Date::afficher(std::ostream& f) const
{
    ...
    f << std::setfill('0') << std::setw(2) << jour << "/" << std::setw(2)
      << mois << "/" << annee << std::setfill(' ');
}
...
{
    if (j > 364)
        throw TimeException("erreur: initialisation periode invalide");
    if (m > 11)
        throw TimeException("erreur: initialisation periode invalide");
}

std::ostream& operator<<(std::ostream& f, const Intervalle& x)
{
    x.afficher(f);
    return f;
}
...
Intervalle::Intervalle(const Date& d, const Date& f) : debut(d), fin(f)
{
    if (fin < debut)
        throw TimeException(
            "Erreur dans la creation d'un intervalle: fin<debut");
}
...
Intervalle Intervalle::operator+(const Intervalle& i) const
{
    ...
    if (d == i.debut)
    {
        ...
        throw TimeException("Ne peut pas faire l'union de 2 intervalles (ils "
                            "ne se touchent pas...");
}
