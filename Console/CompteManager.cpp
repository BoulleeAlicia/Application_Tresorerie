...
void CompteManager::creerCompte(Compte& p, const string& n, Type t, double s, TypeCompte tc)
{
		...
		else if (p.getType()!=racine && p.getType()!=t) throw Erreur("ERREUR : Impossible d'ajouter un compte avec un type différent de son compte père");
		else if (s<0) throw Erreur("ERREUR : Impossible d'ajouter un compte avec un solde initial négatif");
		else if (tc==reel && t!=passif && p.getType()==racine) throw Erreur("ERREUR : Impossible d'ajouter un compte réel non passif sous la racine");
		else if (t!=passif && t!=actif && s>0) throw Erreur("ERREUR : Impossible d'initialiser le solde d'un compte de dépense ou de recette");
		else if (tc==virtuel && s>0) throw Erreur("ERREUR : Impossible d'initialiser le solde d'un compte virtuel");
...
void CompteManager::supprimerCompte(Compte& c)
{
		...
		else if (c.getTypeCompte()==reel && TransactionManager::getInstance().UsageCompte(dynamic_cast<CompteReel&>(c))==0) throw Erreur("ERREUR: Impossible de supprimer un compte qui a des transactions");
		else if (c.getTypeCompte()==virtuel && dynamic_cast<CompteVirtuel&>(c).fils.empty()!=1) throw Erreur("ERREUR: Impossible de supprimer un compte virtuel qui a des fils");
		if (c.getTypeCompte()==reel)
		{
...
Compte& CompteManager::getCompteByNom(const string& n)
{
	...
	if (!existeNom(n))
		throw Erreur("ERREUR: Le compte n'existe pas");
	else return getRacine().getCompteByNom(n);
